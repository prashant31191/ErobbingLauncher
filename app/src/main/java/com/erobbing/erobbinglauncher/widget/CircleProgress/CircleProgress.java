package com.erobbing.erobbinglauncher.widget.CircleProgress;
...
/**
 * Created by zhangzhaolei on 17/7/4.
 */
public class CircleProgress extends ProgressBar {
    ...
    public CircleProgress(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TypedArray attributes = getContext().obtainStyledAttributes(
                attrs, R.styleable.CircleProgress);
        mBoardWidth = attributes.getDimensionPixelOffset(R.styleable.CircleProgress_borderWidth, DEFAULT_LINEHEIGHT);
        mUnmProgressColor = attributes.getColor(R.styleable.CircleProgress_unprogresColor, DEFAULT_mUnmProgressColor);
        mProgressColor = attributes.getColor(R.styleable.CircleProgress_progressColor, DEFAULT_mProgressColor);
        mTickWidth = attributes.getDimensionPixelOffset(R.styleable.CircleProgress_tickWidth, DEFAULT_mTickWidth);
        mTickDensity = attributes.getInt(R.styleable.CircleProgress_tickDensity, DEFAULT_DENSITY);
        mRadius = attributes.getDimensionPixelOffset(R.styleable.CircleProgress_radius, DEFAULT_mRadius);
        mArcbgColor = attributes.getColor(R.styleable.CircleProgress_arcbgColor, DEFAULT_mUnmProgressColor);
        mTickDensity = Math.max(Math.min(mTickDensity, MAX_DENSITY), MIN_DENSITY);
        mBgShow = attributes.getBoolean(R.styleable.CircleProgress_bgShow, false);
        mDegree = attributes.getInt(R.styleable.CircleProgress_degree, DEFAULT_OFFSETDEGREE);
        mStylePogress = attributes.getInt(R.styleable.CircleProgress_progressStyle, STYLE_TICK);
        boolean capRount = attributes.getBoolean(R.styleable.CircleProgress_arcCapRound, false);
    ...
    protected synchronized void onDraw(Canvas canvas) {
        ...
        if (mOnCenter != null) {
            ...
            Bitmap rotateBitmap = BitmapRotation(mCenterBitmap, 180);//zhangzhaolei + rotate 180
    ...
    private Bitmap BitmapRotation(Bitmap bitmap, final int orientationDegree) {
        ...
        Bitmap bitmapRotate = Bitmap.createBitmap(bitmap.getHeight(), bitmap.getWidth(), Bitmap.Config.ARGB_8888);
        Paint paint = new Paint();
        Canvas canvas = new Canvas(bitmapRotate);
